		...
		if config, _, err := image.DecodeConfig(bytes.NewReader(data)); err == nil {
			...
			if info.MimeType == "image/gif" {
				...
				if gifConfig, err := gif.DecodeAll(bytes.NewReader(data)); err != nil {
					...
					err = NewAppError("FileInfo.GetInfoForBytes", "model.file_info.get.gif.app_error", nil, "name="+name, http.StatusBadRequest)
				} else {
...
func GetEtagForFileInfos(infos []*FileInfo) string {
	...
	return Etag(infos[0].CreatorId, maxUpdateAt)
}
