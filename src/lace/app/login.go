...
func (a *App) AuthenticateUserForLogin(id, loginId, password string) (user *model.User, err *model.AppError) {
...
func (a *App) GetUserForLogin(id, loginId string) (*model.User, *model.AppError) {
	enableUsername := *a.Config().LoginSettings.EnableSignInWithUsername
	enableEmail := *a.Config().LoginSettings.EnableSignInWithEmail

	// If we are given a userID then fail if we can't find a user with that ID
	if len(id) != 0 {
		if user, err := a.GetUser(id); err != nil {
			if err.Message != "MISSING_ACCOUNT_ERROR" {
	...
	if result := <-a.UserProvider().GetForLogin(loginId, enableUsername, enableEmail); result.Err == nil {
...
func (a *App) DoLogin(w http.ResponseWriter, r *http.Request, user *model.User, deviceId string) (*model.Session, *model.AppError) {
	session := &model.Session{UserId: user.Id, Roles: user.GetRawRoles(), DeviceId: deviceId}
	session.GenerateCSRF()
	maxAge := a.Config().LoginSettings.SessionLengthWebInDays * 60 * 60 * 24

	if len(deviceId) > 0 {
		session.SetExpireInDays(a.Config().LoginSettings.SessionLengthMobileInDays)
		...
		session.SetExpireInDays(a.Config().LoginSettings.SessionLengthWebInDays)
	}
	...
	w.Header().Set(utils.HEADER_TOKEN, session.Token)
...
func GetProtocol(r *http.Request) string {
	if r.Header.Get(utils.HEADER_FORWARDED_PROTO) == "https" || r.TLS != nil {
		return "https"
	} else {
		return "http"
	}
